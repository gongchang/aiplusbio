"""
Free API Computing Event Searcher
Uses free, user-friendly APIs and RSS feeds to find computing events in Boston area.
No complex authentication required for most sources.
"""
...
class FreeAPIComputingSearcher:
    def __init__(self, db_path='events.db'):
        ...
        # RSS feeds for big tech companies and open source communities
        self.rss_feeds = [
            # Google Developer Groups and Events
            'https://developers.google.com/events/feed',
            'https://cloud.google.com/events/feed',
            'https://developers.googleblog.com/feeds/posts/default',
            
            # Microsoft Developer Events
            'https://devblogs.microsoft.com/feed/',
            'https://azure.microsoft.com/en-us/blog/feed/',
            'https://techcommunity.microsoft.com/events/feed',
    ...
    def search_events(self, max_results: int = 20) -> List[Dict[str, Any]]:
        """Search for computing events using free APIs and RSS feeds"""
        all_events = []
        
        print(f"🔍 Free API search for computing events in Boston area (next 6 months)")
        print(f"📅 Date range: {self.date_range['start_date']} to {self.date_range['end_date']}")
        print(f"🎯 Max results: {max_results}")
        print("-" * 60)
        
        # 1. Search RSS feeds from major institutions
        rss_events = self._search_rss_feeds(max_results // 4)
        all_events.extend(rss_events)
        
        # 2. Search Eventbrite API (if available)
        if self.eventbrite_api_key:
            eventbrite_events = self._search_eventbrite_events(max_results // 4)
            all_events.extend(eventbrite_events)
        else:
            print("⚠️  Eventbrite API key not found - skipping Eventbrite search")
        
        # 3. Search Tavily API with improved queries
        ...
        # 4. Search public event calendars (web scraping)
        calendar_events = self._search_public_calendars(max_results // 4)
    ...
    def _search_rss_feeds(self, max_results: int) -> List[Dict[str, Any]]:
        """Search RSS feeds from major institutions"""
        print("📡 Searching RSS feeds from major institutions...")
        events = []
        ...
        for feed_url in self.rss_feeds:
            try:
                ...
                for entry in feed.entries[:max_results // len(self.rss_feeds)]:
                    event = self._extract_rss_event(entry, feed_url)
                    if event and self._meets_all_criteria(event):
                        events.append(event)
        ...
        print(f"RSS feeds found {len(events)} relevant events")
        return events
    
    def _extract_rss_event(self, entry, feed_url: str) -> Optional[Dict[str, Any]]:
        """Extract event information from RSS entry"""
        try:
            ...
            if not any(keyword.lower() in title.lower() for keyword in self.computing_keywords):
                return None
            ...
            event = {
                ...
                'url': entry.get('link', ''),
                'source_url': feed_url,
                'is_virtual': 'virtual' in title.lower() or 'online' in entry.get('description', '').lower(),
                'requires_registration': 'register' in entry.get('description', '').lower(),
                'categories': self._extract_categories(title),
                'host': self._extract_rss_host(feed_url),
                'cost_type': self._determine_cost_type(entry.get('description', '')),
                'date': self._extract_rss_date(entry),
                'time': self._extract_rss_time(entry),
                'location': self._extract_rss_location(entry),
                'source': 'RSS Feed'
            }
            ...
            print(f"Error extracting RSS event: {e}")
            return None
    
    def _extract_rss_host(self, feed_url: str) -> str:
        """Extract host from RSS feed URL"""
        if 'google.com' in feed_url or 'developers.google' in feed_url:
            return 'Google'
        elif 'microsoft.com' in feed_url or 'azure.microsoft' in feed_url:
            return 'Microsoft'
        elif 'huggingface.co' in feed_url:
            return 'Hugging Face'
        elif 'redhat.com' in feed_url:
            return 'Red Hat'
        elif 'github.com' in feed_url or 'github.blog' in feed_url:
            return 'GitHub'
        elif 'aws.amazon.com' in feed_url:
            return 'Amazon Web Services'
        elif 'nvidia.com' in feed_url:
            return 'NVIDIA'
        elif 'docker.com' in feed_url:
            return 'Docker'
        elif 'kubernetes.io' in feed_url:
            return 'Kubernetes'
        elif 'cncf.io' in feed_url:
            return 'CNCF'
        elif 'dev.to' in feed_url:
            return 'DEV Community'
        elif 'stackoverflow.com' in feed_url:
            return 'Stack Overflow'
        elif 'infoq.com' in feed_url:
            return 'InfoQ'
        elif 'thenewstack.io' in feed_url:
    ...
    def _extract_rss_time(self, entry) -> str:
        """Extract time from RSS entry"""
        try:
            if hasattr(entry, 'published_parsed') and entry.published_parsed:
                dt = datetime(*entry.published_parsed[:6])
    ...
    def _extract_rss_location(self, entry) -> str:
    ...
    def _search_eventbrite_events(self, max_results: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            params = {
                ...
                'q': 'AI OR machine learning OR cloud computing OR devops OR bioinformatics',
    ...
    def _search_tavily_events(self, max_results: int) -> List[Dict[str, Any]]:
        """Search for events using Tavily API with targeted queries"""
        print("🔍 Searching Tavily API with targeted queries...")
        
        # Multiple targeted searches for tech company events
        search_queries = [
            'Google Developer Groups Boston 2025 OR "Google Cloud" events Massachusetts OR "Google AI" workshop Cambridge',
            'Microsoft events Boston 2025 OR "Azure" conference Massachusetts OR "Microsoft Developer" meetup Cambridge',
            'Hugging Face events Boston 2025 OR "Red Hat" conference Massachusetts OR "GitHub" workshop Cambridge',
            '"AWS" events Boston 2025 OR "Amazon Web Services" conference Massachusetts OR "NVIDIA" workshop Cambridge',
            'Docker events Boston 2025 OR "Kubernetes" conference Massachusetts OR "CNCF" workshop Cambridge'
        ]
        
        all_events = []
        for i, query in enumerate(search_queries, 1):
            print(f"  🔍 Tavily search {i}/5: {query[:60]}...")
            events = self._search_tavily_with_query(query, max_results // len(search_queries))
            all_events.extend(events)
        
        print(f"Tavily found {len(all_events)} relevant events")
        return all_events
    ...
    def _search_public_calendars(self, max_results: int) -> List[Dict[str, Any]]:
        """Search public event calendars using web scraping"""
        print("🌐 Searching public event calendars...")
        events = []
        
        # List of tech company and developer community event URLs
        calendar_urls = [
            'https://gdg.community.dev/events/#/list',
            'https://events.google.com/',
            'https://events.microsoft.com/',
            'https://www.meetup.com/find/events/?location=us--ma--boston&categoryId=34',
            'https://www.eventbrite.com/d/ma--boston/technology/',
            'https://www.meetup.com/find/events/?location=us--ma--cambridge&categoryId=34'
        ]
        
        for url in calendar_urls:
            try:
                print(f"  🌐 Checking: {url}")
                # Simple web scraping (you might want to use BeautifulSoup for more complex parsing)
                response = requests.get(url, timeout=10)
                if response.status_code == 200:
                    # This is a simplified approach - in practice you'd parse the HTML
                    # For now, we'll skip detailed scraping to avoid complexity
                    pass
            except Exception as e:
                print(f"  ❌ Error with calendar {url}: {e}")
                continue
        
        print(f"Public calendars found {len(events)} relevant events")
        return events
...
def main():
    ...
    searcher = FreeAPIComputingSearcher()
    
    print("🔍 Free API search for computing events in Boston area...")
