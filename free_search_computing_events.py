#!/usr/bin/env python3
"""
Free API command line interface for searching computing events.
Uses RSS feeds, Eventbrite API, and Tavily API for comprehensive event discovery.
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Free API search for computing events in Boston area using RSS feeds, Eventbrite, and Tavily')
    ...
    if missing_keys:
        ...
        for key in missing_keys:
            print(f"   - {key}")
        print("\nThe search will continue with available sources only.")
    ...
    print(f"🚀 Free API search for computing events in Boston area...")
    print(f"📅 {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"🎯 Max results: {args.max_results}")
    print(f"💾 Save to database: {'No' if args.dry_run else 'Yes'}")
    print(f"📊 Using sources: RSS Feeds✅, Eventbrite{'✅' if os.getenv('EVENTBRITE_API_KEY') else '❌'}, Tavily{'✅' if os.getenv('Tavily_API') else '❌'}")
    print("-" * 60)
    
    try:
        # Initialize free API searcher
        searcher = FreeAPIComputingSearcher()
        
        # Search for events using free APIs and RSS feeds
        ...
        print("\n✅ Free API search completed successfully!")
